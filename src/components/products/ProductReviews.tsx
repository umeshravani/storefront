...
const THEME = {
  ...
  button:
    "bg-black hover:bg-gray-800 dark:bg-white dark:text-black dark:hover:bg-gray-200 focus:ring-gray-300 dark:focus:ring-gray-700",
  ...
  inputFocus:
    "focus:border-black focus:ring-black dark:focus:border-gray-500 dark:focus:ring-gray-500",

  // Checkboxes & Radios (Checked color and focus rings)
  checkbox:
    "text-black focus:ring-black dark:checked:bg-gray-300 dark:focus:ring-gray-500",
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        if (NEXT_PUBLIC_SPREE_API_KEY)
          headers["X-Spree-Api-Key"] = NEXT_PUBLIC_SPREE_API_KEY;
        ...
        if (json.data) {
          ...
          const fetchedReviews = json.data.filter(
            (r: any) => r.approved === true,
          );
          setReviews(fetchedReviews);

          if (fetchedReviews.length > 0) {
            let sum = 0;
            const dist = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 } as Record<
              number,
              number
            >;
    ...
    fetchReviews();
    // biome-ignore lint/correctness/useExhaustiveDependencies: legacy code
  }, [slug, NEXT_PUBLIC_SPREE_API_KEY, getActiveToken]);
      ...
      if (res.ok) {
        ...
        if (errorData.error) {
          if (typeof errorData.error === "string") {
            ...
            errorMsg = Object.entries(errorData.error)
              .map(([key, val]) => `${key} ${val}`)
              .join("\n");
          }
        }
              ...
              {[5, 4, 3, 2, 1].map((stars) => {
                ...
                return (
                  <div key={stars} className="flex items-center gap-2">
                    ...
                    <span
                      className={`w-8 shrink-0 text-right text-sm font-medium leading-none hover:underline sm:w-auto sm:text-left ${THEME.link}`}
                    >
              ...
              <div className="shrink-0 space-y-2 sm:w-48 md:w-72">
                ...
                {review.is_verified_purchase && (
                  <div className="inline-flex items-center gap-1">
                    <svg
                      className={`h-5 w-5 ${THEME.verifiedBadge}`}
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="currentColor"
                      viewBox="0 0 24 24"
              ...
              <div className="mt-4 min-w-0 flex-1 space-y-4 sm:mt-0">
                            ...
                            <>
                              ...
                              <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-colors">
                                <svg
                                  ...
                                  viewBox="0 0 24 24"
                                  fill="currentColor"
                                  className="w-8 h-8 text-white drop-shadow-md opacity-90"
                                >
                                  <path
                                    stroke="none"
                                    d="M0 0h24v24H0z"
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center">
                    <input
                      id={`reviews-radio-yes-${review.id}`}
                      type="radio"
                      value="yes"
                      name={`reviews-radio-${review.id}`}
                      className={`h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-2 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 ${THEME.checkbox}`}
                    />
                    <label
                      htmlFor={`reviews-radio-yes-${review.id}`}
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <label
                      htmlFor={`reviews-radio-no-${review.id}`}
                      className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                    >
                      {" "}
                      No{" "}
              ...
              >
                <div className="mb-4 grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      id="title"
                      className={`block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 outline-none ${THEME.inputFocus}`}
                      required
                    />
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      name="product_review[review]"
                      id="description"
                      rows={6}
                      className={`mb-2 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 outline-none ${THEME.inputFocus}`}
                      required
                    ></textarea>
                    <p className="ms-auto text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <a href="#" className={`hover:underline ${THEME.link}`}>
                        Send a report
                      </a>
                      .
                    </p>
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="flex w-full items-center justify-center">
                      <label
                        ...
                        className="dark:hover:bg-gray-800 flex h-52 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                      >
                        <div className="flex flex-col items-center justify-center pb-6 pt-5">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            PNG, JPG or Video (Max 50MB)
                          </p>
                          {selectedFilesCount > 0 && (
                            <p
                              className={`mt-2 text-sm font-bold ${THEME.textHighlight}`}
                            >
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="flex items-center mt-2">
                      <input
                        type="checkbox"
                        id="show_identifier"
                        name="product_review[show_identifier]"
                        value="1"
                        defaultChecked
                        className={`h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-2 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 ${THEME.checkbox}`}
                      />
                      <label
                        htmlFor="show_identifier"
                        className="ms-2 text-sm font-medium text-gray-500 dark:text-gray-400"
                      >
                ...
                <div className="border-t border-gray-200 pt-4 dark:border-gray-700 md:pt-5">
                  <button
                    ...
                    className={`me-2 inline-flex items-center rounded-lg px-5 py-2.5 text-center text-sm font-medium text-white focus:outline-none focus:ring-4 disabled:opacity-50 transition-colors ${THEME.button}`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="me-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                  >
                    Cancel
                  </button>
                </div>
        ...
        >
          ...
          {lightbox.media.length > 1 && (
            <>
              <button
                type="button"
                className="absolute left-4 top-1/2 -translate-y-1/2 text-white p-2 hover:bg-white/10 rounded-lg transition-colors z-10"
              ...
              <button
                type="button"
                className="absolute right-4 top-1/2 -translate-y-1/2 text-white p-2 hover:bg-white/10 rounded-lg transition-colors z-10"
          ...
          <div
            className="relative max-w-4xl max-h-[90vh] w-full h-full m-4 flex items-center justify-center"
            onClick={(e) => e.stopPropagation()}
          >
            {lightbox.media[lightbox.currentIndex].content_type.startsWith(
              "video",
            ) ? (
              // biome-ignore lint/a11y/useMediaCaption: no captions available
              <video
